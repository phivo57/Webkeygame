...
if(isset($_GET["ma_game"])){
    ...
    $m_game=new game();
    $kq=$m_game->Sua_game($ma_game);
    
    if($kq){
        header("location:sua_sp.php");
    }
...
if(isset($_GET["ma_game"]))
{
    ...
    if(isset($_POST['sua']))
    {
        ...
        $update = new game();
        $up = $update->Sua_game($key_game,$ma_the_loai,$ten_game,$don_gia_nhap,$don_gia_ban,$mo_ta_tom_tat,$mo_ta_chi_tiet,$cau_hinh,$cau_hinh_yeu_cau,$ngay_nhap,$ma_trang_thai,$hinh,$ma_game);
        if($up)
        {
            ...
            echo "<script>alert('Cập nhật sản phẩm thành công');window.location=`sua_sp.php`</script>";
        }
        else
        {
            echo "<script>alert('Cập nhật sản phẩm không thành công');window.location=`sua.php?ma_game=$ma_game`</script>";
        }
    }
}
?>
